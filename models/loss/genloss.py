...
class GeneralizedLoss(nn.modules.loss._Loss):
    ...
    def den2coord(self, denmap):
        ...
        #使用torch.nonzero方法找到denmap中非零元素的索引坐标。这个方法返回一个张量，n行2列，其中每一行都是找到的非零元素的坐标。
        coord = torch.nonzero(denmap)
        #使用这些坐标索引获取denmap中对应位置的值，并将这些值存储在一个名为denval的张量中,2个n列向量。
