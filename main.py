...
from vis_helper import Visualizer
...
def main_worker(config):
    ...
    total_parameters = sum(p.numel() for p in model_without_ddp.parameters())
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info(f"total number of params: {total_parameters}")
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, lr_scheduler):
    ...
    for idx, (images, boxes, targets, imgids) in enumerate(data_loader):
        images = images.cuda(non_blocking=True)
        boxes = boxes.cuda(non_blocking=True)
        targets = targets.cuda(non_blocking=True)

        sdenmap = model(images, boxes)
...
def validate(config, data_loader, model, criterion):
    ...
    for idx, (images, boxes, target, imgids) in enumerate(data_loader):
        ...
        vis = Visualizer()
        vis.vis_batch(imgids, target, output)
